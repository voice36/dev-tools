// DevTools API worker — keeps upstream API keys OFF the browser.
// Route on the same origin so the page can call "/api/balance" with no CORS:
//   tools.vexum.top/api/*
//
// Required secret:  ETHERSCAN_API_KEY   (free: https://etherscan.io/myapikey)
// Optional secret:  TRON_PRO_API_KEY    (faster TRON; TronGrid works without one)
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname !== "/api/balance") return json({ error: "not_found" }, 404);
